...
def echo(device, qubit_id, transition='01', *extra_sweep_args, channel_amplitudes=None, lengths=None,
    ...
    excitation_channel = [i for i in device.get_qubit_excitation_channel_list(qubit_id, transition=transition).keys()][
        0]
    ...
    class ParameterSetter:
        ...
        def virtual_phase(self, length):
            ...
            """
            phase = int(np.round(
                length * self.control_sequence.clock)) / self.control_sequence.clock * target_freq_offset * 360 % 360
            return phase
...
''')
        ...
        def set_delay(self, length):
            ...
            for seq in ex_sequencers:
                ...
                seq.set_length(int(round(length / 2 * control_sequence.clock)) / control_sequence.clock)
                print('delay is {}, and delay / 2 is {}'. format(length, int(round(length / 2 * control_sequence.clock)) / control_sequence.clock))
...
// Table entry play part
    ...
    //executeTableEntry(0);
    //resetOscPhase();
''').format(num_pulses=self.num_pulses)
            self.instructions.append(command_table)
            print('Command table for sequencer id {}'.format(ex_seq.params['sequencer_id']), command_table)
            return definition_part, play_part
        ...
        def set_delay(self, length):
            ...
            for seq in ex_sequencers:
                seq.set_length(int(round(length / 2 / self.num_pulses * control_sequence.clock)) / control_sequence.clock)
                print('delay is {}, and delay / 2 is {}'.format(length, int(round(
                    length / 2 * control_sequence.clock)) / control_sequence.clock))
