...
def get_rect_cos_pulse_sequence(device, channel_amplitudes, tail_length, length, phase = 0, fast_control=False):
    ...
    '''Warning'''
    return [device.pg.pmulti(device, length + 2 * tail_length, *tuple(channel_pulses))]
...
def get_excitation_pulse_from_Rabi_rect_fit(device, rotation_angle, Rabi_fit):
    ...
    metadata = {'qubit_id': Rabi_rect_measurement.metadata['qubit_id'], 'rotation_angle': rotation_angle,
                'calibration_type': 'Rabi_rect'}
    channel_amplitudes = device.exdir_db.select_measurement(measurement_type='channel_amplitudes', references_this={
        'channel_amplitudes': Rabi_rect_measurement.id})
...
def get_rect_excitation_pulse(device, qubit_id, rotation_angle, transition='01', channel_amplitudes_override=None,
    ...
    for attempt_id in range(2):
        ...
        query = Rabi_measurements_query(qubit_id=qubit_id,
                                        ...
                                        frequency_tolerance=device.get_qubit_constant(qubit_id=qubit_id,
                                                                                      name='frequency_rounding'),
                                        frequency_controls=device.get_frequency_control_measurement_id(
                                            qubit_id=qubit_id),
                                        channel_amplitudes_override=channel_amplitudes_override.id if hasattr(
        ...
        if recalibrate:
            if channel_amplitudes_override is None:
                ...
                Rabi.Rabi_rect_adaptive(device, qubit_id=qubit_id, transition=transition,
                                        channel_amplitudes=channel_amplitudes_override)
        else:
...
def Rabi_measurements_query(qubit_id, transition, frequency, frequency_tolerance,
                            frequency_controls, sample_name, channel_amplitudes_override=None,
                            channel_calibration=False):
    '''
