...
def get_qubit_readout_pulse_from_passthrough(device, passthrough_measurement):
    print('GET QUBIT READOUT PULSE FROM PASSTHROUGH!')
...
def get_multi_qubit_readout_pulse(device, qubit_ids, length=None, ampl=None):
    pulses = {}
    references = {}
    if not len(qubit_ids):
        raise ValueError ('No qubits to readout!')
    for qubit_id in qubit_ids:
        pulses[qubit_id] = get_qubit_readout_pulse(device, qubit_id, length, ampl)
    ...
    if len(qubit_ids)==1:
        if not ampl:
            ...
            multi_readout_pulse.definition_fragment, multi_readout_pulse.play_fragment = device.pg.readout_rect(pg_args[0][0], float(length), ampl)
    else:
...
def get_qubit_readout_pulse(device, qubit_id, length=None, ampl=None):
...
def get_uncalibrated_measurer(device, qubit_id, transition='01', samples=False, shots=False, dot_products=False,
                              readouts_per_repetition=1, qutrit_readout=False):
    """
    :param device:
    :param qubit_id:
    :param transition:
    ...
    try:
       ...
       if not qutrit_readout:
           ...
           qubit_readout_pulse_, measurer = get_qutrit_calibrated_measurer(device, [qubit_id], recalibrate=False)
       print (measurer.avg_cov_mode)
       reducer = data_reduce.data_reduce(measurer)
       #
       # print('measurer', dir(measurer))
       # print(measurer.get_points())
       ...
       if dot_products:
           ...
           # reducer.filters['dot_products0'] = data_reduce.thru(measurer, 'disc_ch0')
       ...
       if qutrit_readout:
           measurer.resultnumber_states = True
           reducer.filters['resultnumbers_states'] = data_reduce.thru(measurer, 'resultnumbers_states')
       else:
           measurer.resultnumber_states = False
    ...
    nums = int(device.get_qubit_constant(name='uncalibrated_readout_nums', qubit_id=qubit_id))
    adc_reducer.set_adc_nums(nums * readouts_per_repetition)
