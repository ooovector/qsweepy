...
def CPhase_calibration_delay(device, qubit_ids, gate, identity_gate, amplitudes, length, transition='01', phis=None,
                             tail_length=0e-9,
                             readout_delay=0e-9, *extra_sweep_args, repeats=1, identity_repeats=[0],
                             pulse_U=None, sign=False,
                             additional_metadata={}, gate_freq=None, bipolar_pulse=False):
    """
    quantum circuit
    if not bipolar_pulse:
        q0 ----I/X----[--CZ(01)--[I(01)]xM--]---------------I/X----
        q1 --Rx(90)--[----------------------]xN--Rz(phi)--Rx(90)--Meas
    elif bipolar_pulse:
        q0 ----I/X----[--CZ(01)--[I(01)]xM--CZ^(-1)(01)--[I(01)]xM--]----------------I/X----
        q1 --Rx(90)--[------------------------------------------------]xN--Rz(phi)--Rx(90)--Meas
    here N = repeats           (default 1)
         M = identity_repeats  (default [0])
    ...
    channel_amplitudes_identity = channel_amplitudes.channel_amplitudes(device,
                                                                        **{identity_gate.metadata['carrier_name']:
                                                                        complex(identity_gate.metadata['amplitude'])})
    ...
    class ParameterSetter:
        def __init__(self):
            ...
            self.frequency = 0
            self.sign = sign
            self.n_identity_gate = identity_repeats[0]
            ...
            gate_sequence_identity = excitation_pulse.get_rect_cos_pulse_sequence(device=device,
                                                                                  channel_amplitudes=channel_amplitudes_identity,
            ...
            self.prepare_seq.append(
                device.pg.parallel(self.pre_pulse2.get_pulse_sequence(0)[0], self.pre_pulse1.get_pulse_sequence(0)[0]))

            self.prepare_seq.extend(gate_sequence)
            self.prepare_seq.extend(gate_sequence1)
            self.prepare_seq.extend(gate_sequence_identity)
            # self.prepare_seq.append(self.pre_pulse1.get_pulse_sequence(0)[0])
            '''
            table_entry:
            2    : Rx(90) q1
        ...
        def n_identity_gate_setter(self, n_identity_gate):
            self.n_identity_gate = n_identity_gate
                ...
                if repeats != 0:
                    play_part += textwrap.dedent('''
                                            ...
                                            for (j=0;j<{repeats}; j++){{
                                                executeTableEntry(1);
                                                executeTableEntry(5);
                                                for (k=0;k<{repeats_identity}; k++){{
                                                    executeTableEntry(7);
                                                }}
                                                wait(5);
                                            }}
                                            executeTableEntry(1);
                                            executeTableEntry(8+variable_register2);
                                            executeTableEntry(1);
                                        ''').format(repeats=repeats, repeats_identity=self.n_identity_gate)
                else:
                    play_part += textwrap.dedent('''
                                            cvar j;
                                            cvar k;
                                            ...
                                            executeTableEntry(1);
                                        ''').format(repeats=1,
                                                    repeats_identity=self.n_identity_gate)
            else:
                play_part += textwrap.dedent('''
                                            cvar j;
                                            cvar k;
    ...
    measurement_type = 'CPhase_calibration_delay'
    exp_sin_fitter_mode = 'unsync'
    measurement_name = 'iq' + qubit_id
    ...
    metadata = {'qubit_id': ','.join(qubit_id),
                ...
                'transition': transition,
                'U': pulse_U,
                'bipolar_pulse' : str(bipolar_pulse),
                'control_type': 'command table'}
    metadata.update(additional_metadata)
    ...
    measurement = device.sweeper.sweep_fit_dataset_1d_onfly(measurer,
                                                            *extra_sweep_args,
                                                            (identity_repeats, setter.n_identity_gate_setter, 'Identity gates', 'num'),
                                                            (phis, setter.phi_setter, 'Z phase', 'rad'),
                                                            fitter_arguments=fitter_arguments,
                                                            measurement_type=measurement_type,
                                                            metadata=metadata,
