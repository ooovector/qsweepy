...
def benchmarking_pi2_multi(device, qubit_ids, *params, interleaver=None, two_qubit_gate=None, max_pulses=None,
    ...
    if max_pulses is None:
        ...
        for qubit_id in qubit_ids:
            ...
            if preferred_excitation_length is not None:
    ...
    for qubit_id in qubit_ids:
        if preferred_excitation_length is not None:
            pi2_pulses[qubit_id] = excitation_pulse.get_excitation_pulse(device, qubit_id, np.pi / 2., preferred_length = preferred_excitation_length)
            pi_pulses[qubit_id] = excitation_pulse.get_excitation_pulse(device, qubit_id, np.pi, preferred_length = preferred_excitation_length)
        else:
            pi2_pulses[qubit_id] = excitation_pulse.get_excitation_pulse(device, qubit_id, np.pi / 2.)
            pi_pulses[qubit_id] = excitation_pulse.get_excitation_pulse(device, qubit_id, np.pi)
...
def benchmarking_pi2(device, qubit_id, *params, pause_length=0, random_sequence_num=1, seq_lengths_num=400, preferred_excitation_length=None):
    coherence_measurement = Ramsey.get_Ramsey_coherence_measurement(device, qubit_id)
    T2 = float(coherence_measurement.metadata['T'])
