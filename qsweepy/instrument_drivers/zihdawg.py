...
class ZIDevice():
    ...
    def set_sequencer(self, sequencer):
        ...
        if len(nonzero) == 0:
            ...
            ch1 = np.asarray((waveformI_truncated + offsetI) * (2 ** 13 - 1) , dtype=np.int16)  # TODO check if this sets to 0 or to 0.001
            ch2 = np.asarray((waveformQ_truncated + offsetQ) * (2 ** 13 - 1) , dtype=np.int16)
