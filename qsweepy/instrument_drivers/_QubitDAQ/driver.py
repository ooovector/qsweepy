...
"""CyUSB3.sys native driver interface"""
import qsweepy.instrument_drivers._QubitDAQ.cyusb3_intf as usb

from qsweepy.instrument_drivers._QubitDAQ.reg_intf import *
from qsweepy.instrument_drivers._QubitDAQ.usb_const import *
...
#Debug mode without ADC board
NO_ADC_DEBUG_MODE = False
...
class Device:
    ...
    """
    def __init__(self, serial: str, fpga_config: bool = True, firmware: str = None) -> None:
        ...
        delete previously created object first using the example code below.
        ...
        self.threshold = np.zeros(NUM_DESCR_CH, dtype = np.int64)
        self.trig_mode = "ext"
    ...
    def i2c_reset(self) -> None:
        ...
        """
        self.dev.ctrl_write( VendorRq.RESET_I2C, 0, 0)
    ...
    def fpga_config(self) -> None:
        """Performs FPGA configuration using *.rbf firmware file

        This method loads configuration into the FPGA from *.rbf file specified in the self.fpga_firmware attribute.
        First it's tries to access the FPGA register with CRC32 checksum of the firmware currently loaded. If the 
        register is unavailable or the checksum doesn't match the checksum of the *.rbf file, then 
        ...
        try:
            ...
            warnings.warn("QubitDAQ: Unable to read FPGA firmware file. The device may be unprogrammed!")
            return
        ...
        if dev_checksum != checksum:
            ...
            value, index = usb.mk_val_ind( int(len(firmware_data)/4)+1 )
            res = self.dev.ctrl_read( VendorRq.FPGA_CONF_INIT, value, index, 2)
            ...
            self.dev.gpif_write( firmware_data + bytes([0,0,0,0]) )
            res = self.dev.ctrl_read( VendorRq.FPGA_CONF_FIN, 0, 0, 2 )
            n_status = res[0] #FPGA status pin
            conf_done = res[1] #FPGA status pin
            if conf_done != 1:
    ...
    def set_ref_100mhz(self)->None:
        """Sets external reference frequency of the ADC board to 100MHz"""
        self.ads = ADS54J40()
    ...
    def set_ref_10mhz(self)->None:
        """Sets external reference frequency of the ADC board to 10MHz"""
        self.ads = ADS54J40()
    ...
    def write_reg(self ,address : int , raw_data: any) -> None:
        ...
        if hasattr(raw_data, '__len__') :
            data = np.frombuffer( np.array(raw_data), dtype = np.uint8)
        else:
            data = np.frombuffer( np.uint32(raw_data), dtype = np.uint8)

        Value, Index = usb.mk_val_ind( address )
        bytes_written= self.dev.ctrl_write( VendorRq.REG_WRITE, Value, Index, bytes(data) )
        if bytes_written != len(data):
            raise Exception("Register write faild!")

    def read_reg(self, address: int, dtype = np.uint32) -> int:
        ...
        Value, Index = usb.mk_val_ind(address)
        raw_data = self.dev.ctrl_read( VendorRq.REG_READ, Value, Index, length)
        if len(raw_data) != length:
            raise Exception("Register read faild!")
    ...
    def read_ddr3(self, base_addr: int, length: int, dtype = np.int16 ) -> np.ndarray:
        ...
        return np.frombuffer( self.dev.gpif_read( ddr_dma_len*64 )[0:byte_len], dtype = dtype )

    def write_ddr3(self, base_addr: int, data: any) -> None:
        ...
        self.dev.gpif_write( data_bytes )
    ...
    def get_dot_prods(self):
        ...
        """
        return np.reshape( self.read_ddr3(DDR_RESULT_BASE, NUM_DESCR_CH * self.nsegm, dtype = np.int64), (self.nsegm, NUM_DESCR_CH
                                                                                                       ) )
    ...
    def read_onchip_memory(self, base_addr:int, length:int, dtype = np.int16) -> np.ndarray:
        ...
        return np.frombuffer( self.dev.gpif_read( bytes_to_read )[0:byte_len], dtype = dtype )
    ...
    def set_threshold(self, threshold: np.int64, chan: int) -> None:
        ...
        self.write_reg( PULSE_PROC_BASE + PULSE_PROC_THRESHOLD + 8*chan, [np.int64(threshold),] )

    def get_threshold(self, chan: int) -> np.int64:
        ...
        if chan >= NUM_DESCR_CH or chan < 0:
            raise ValueError("Channel number is out of range!")
        return self.read_reg( PULSE_PROC_BASE + PULSE_PROC_THRESHOLD + 8*chan, dtype = np.int64 )
    ...
    def get_dot_prod_ave(self) -> np.ndarray:
        ...
        Value, Index = usb.mk_val_ind(PULSE_PROC_BASE + PULSE_PROC_DOT_AVE)
        raw_data = self.dev.ctrl_read(VendorRq.REG_READ, Value, Index, length)
        #raw_data = self.dev.ctrl_transfer(RQ_TYPE_DEV_VEND_RD, VendorRq.REG_READ, Value, Index, length)
