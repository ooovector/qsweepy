...
class data_reduce:
	...
	def get_opts(self):
		return {filter_name: {**filter['get_opts'](), **self.extra_opts} for filter_name, filter in
				self.filters.items()}
...
def downsample_reducer(source, src_meas, axis, carrier, downsample, iq=True, iq_axis=-1):
	...
	intermediate_axes = [len(a[1]) for a in source.get_points()[src_meas][:axis]] + [
		len(source.get_points()[src_meas][axis][1][::downsample]), downsample] + [len(a[1]) for a in
																				  source.get_points()[src_meas][
																				  axis + 1:]]
	filter_func = lambda x, s: np.mean(
		np.reshape(np.exp(s * 2 * np.pi * 1j * source.get_points()[src_meas][axis][1] * carrier) * x[src_meas],
				   intermediate_axes), axis=axis + 1)

	filter = {
		'filter': lambda x: filter_func(x, 1) if not iq else np.concatenate([filter_func(x, 1), filter_func(x, -1)],
																			axis=iq_axis),
...
def feature_reducer(source, src_meas, axis_mean, bg, feature):
	...
	def filter_func(x):
		feature_truncated_shape = tuple([slice(None) if i != axis_mean else slice(x[src_meas].shape[axis_mean]) for i in
										 range(len(new_feature_shape))])
...
def post_selection_filter(source, src_meas, axis, threshold):
	"""
	Post select data with known threshold usinf postselection function
	:param source:
	:param src_meas:
	:param threshold:
	:param meas0_state: value of the first measurement
	"""
	def get_points():
		new_axes = source.get_points()[src_meas].copy()
		del new_axes[axis]
