...
class multiqubit_tomography:
    ...
    def return_hdawg_program(self, ex_seq):
        ...
        if self.prepare_seq is not None:
            if self.middle_pulse is not None:
                two_qubit_gate_index = random_command_id - 1-len(self.middle_pulse)
                # two_qubit_middle_gate_index = random_command_id - 1
...
//  Tomography play part
    ...
    //Two qubit first sequence
    executeTableEntry({random_gate_num}+1);
    executeTableEntry({two_qubit_gate_index});
    executeTableEntry({random_gate_num}+1);'''.format(two_qubit_gate_index=two_qubit_gate_index, random_gate_num=random_gate_num))

                for _i in range(len(self.middle_pulse)):
                    two_qubit_middle_gate_index = two_qubit_gate_index + 1 + _i
                    play_part += textwrap.dedent('''
    
// Middle pulse
    executeTableEntry({two_qubit_middle_gate_index});'''.format(two_qubit_middle_gate_index=two_qubit_middle_gate_index))

                play_part += textwrap.dedent('''
// Two qubit second sequence
    executeTableEntry({random_gate_num}+1);
    executeTableEntry({two_qubit_gate_index});
    //Pulses
    executeTableEntry({random_gate_num}+1);
...
resetOscPhase();'''.format(random_gate_num=random_gate_num))
    ...
    def set_middle_pulse(self, seq):
        self.middle_pulse = seq
        for name, gate in self.middle_pulse.items():
            self.add_interleaver(name, gate['pulses'], gate['unitary'])
