...
class AWGIQMulti:
    ...
    Attributes:
        awg (:obj:`awg`): Instance of an AWG class (for example, hdawg)
            that is connected to the I and Q inputs of the mixer. Should implement the methods get_clock,
            set_clock, set_status, run and stop.
        channels (int): channels id that mixer is connected to
        lo (:obj:`psg`): Instance of a sinusoidal signal generator. Should implement the methods get_frequency and set_frequency.
    """

    def __init__(self, awg, channel_i, channel_q, lo, exdir_db):
        """
        """
        self.awg = awg
    ...
    def set_status(self, status):
        ...
        #TODO: set status to awg device channels
        self.awg.set_status(status, channel=self.channel_i)
        self.awg.set_status(status, channel=self.channel_q)
    ...
    def do_calibration(self, sa=None):
        ...
        frequencies.
        """
        #TODO: add righr value of calibrated i and q channels
    ...
    def _calibrate_cw_sa(self, sa, carrier, num_sidebands = 3, use_central = False, num_sidebands_final = 9, half_length = True, use_single_sweep=False):
        ...
        # TODO: set parameters to awg device
        ...
        def tfunc(x):
            ...
            if np.abs(Q) >= 0.49:
                Q = Q/np.abs(Q)*0.49
            # TODO: set parameters to awg device for rf calibration
    ...
    def _calibrate_zero_sa(self, sa):
        ...
        self.lo.set_status(True)
        def tfunc(x):
            # TODO: set dc values to I and Q channels
