from qsweepy.libraries_dorf import awg_iq_multi as awg_iq_multi
from qsweepy.libraries_dorf import awg_channel as awg_channel
from qsweepy.libraries_dorf import modem_readout as modem_readout
...
class QubitDevice:
    ...
    def set_qubits_from_dict(self, _dict: Mapping):
        """
        Set qubits from dictionary:
        :param _dict: dictionary of qubits
        """
        try:
    ...
    def set_sample_globals(self, globals: Mapping):
        """
        Set sample globals
        :param globals: dictionary of sample globals
        """
        ...
        for _qubit_id in self.get_qubit_list():
            for channel_name, device_name in self.get_qubit_excitation_channel_list(_qubit_id, None).items():
                if device_name in iq_devices:
                    ...
                    device.carriersdevice.carriers[channel_name] = carrier
                else:
