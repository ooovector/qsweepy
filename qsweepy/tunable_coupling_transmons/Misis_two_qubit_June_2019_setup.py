...
pulsed_settings = {'lo1_power': 18,
				   ...
				   'awg2_ch1_amplitude':0.5,
				   'awg2_ch2_amplitude':0.5,
				   'awg2_ch3_amplitude':0.5,
...
class hardware_setup():
	...
	def set_pulsed_mode(self):
		...
		self.awg1.set_clock_output(True)
		...
		self.awg1.module.waveforms = [None]*4
		self.awg1.module.AWGflush(nAWG=0) #убирает waveforms, которые были на этом канале
		self.awg1.module.AWGflush(nAWG=1)
		self.awg1.module.AWGflush(nAWG=2)
		self.awg1.module.AWGflush(nAWG=3)
		self.awg2.module.waveforms = [None]*4
		self.awg2.module.AWGflush(nAWG=0)
		self.awg2.module.AWGflush(nAWG=1)
		self.awg2.module.AWGflush(nAWG=2)
		self.awg2.module.AWGflush(nAWG=3)

		# а вот длину сэмплов, которая очевидно то же самое, нужно задавать на всех авгшках.
		# хорошо, что сейчас она только одна.
		#this is zashkvar
		self.awg2.set_trigger_mode(1) #external trigger mode
		self.awg1.set_trigger_mode(1)

		self.awg1.trigger_source_channels = [0, 0, 0, 0]
		self.awg2.trigger_source_channels = [0, 0, 0, 0]#=0 for external trigger
		#awg2.trigger_delays = [30+10,7+10,7+10,7+10]
		self.awg1.trigger_delays = [1,1,1,1]
		self.awg2.trigger_delays = [1,1,1,1]

		self.awg1.trigger_behaviours = [4,4,4,4] #Trigger is active on the falling edge
		self.awg2.trigger_behaviours = [4,4,4,4]

		self.awg1.trigger_source_types = [6,6,6,6] # External HARDWARE trigger per cycle
		self.awg2.trigger_source_types = [6,6,6,6]

		for channel in range(0,4):
			self.awg2.set_offset(0.0,channel=channel)
			self.awg2.set_output(1, channel=channel)
			self.awg2.set_waveform(waveform = [0]*global_num_points, channel=channel)
		for channel in range(0,4):
		...
		self.awg1.set_amplitude(self.pulsed_settings['awg1_ch3_amplitude'], channel=3)
		self.awg2.set_amplitude(self.pulsed_settings['awg2_ch0_amplitude'], channel=0)
		...
		self.awg_tek.set_digital([1]*90+[0]*(global_num_points-90), channel=1) # triggers PXI modules
		self.awg_tek.set_digital([1]*90+[0]*(global_num_points-90), channel=2) #
