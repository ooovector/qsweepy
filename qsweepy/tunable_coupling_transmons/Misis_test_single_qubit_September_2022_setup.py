...
pulsed_settings = {'lo1_power': 15,
                   ...
                   'lo1_freq': 5.35e9, #5.5e9, #5.53e9, #7.75e9, #5.45e9,# frequency or dc excitation calibration
                   'pna_freq': 7.3e9, #6.2e9, #7.34e9, #8.45e9, #6.55e9, # frequency or dc readout calibration
...
class hardware_setup():
    ...
    def open_devices(self):
        ...
        # HDAWG Zurich
        self.hdawg = instruments.ZIDevice(self.device_settings['hdawg1_address'], devtype='HDAWG', delay_int=0)

        for channel_id in range(qubits_nu):
        ...
        # # We use only one channel for qubit flux because we use a switch
        self.q1z = awg_channel(self.hdawg, 4)
        self.q2z = awg_channel(self.hdawg, 4)
        self.q3z = awg_channel(self.hdawg, 4)
        self.q4z = awg_channel(self.hdawg, 4)
        self.q5z = awg_channel(self.hdawg, 4)
        self.q6z = awg_channel(self.hdawg, 4)

        self.hdawg.set_output(4, 1)
        ...
        # For single and two tone spectroscopy we use four channels from Tektronix and two channels from hdawg
        # self.q1z = awg_channel(self.awg_tek, 1)
        # self.q2z = awg_channel(self.awg_tek, 2)
        # self.q3z = awg_channel(self.awg_tek, 3)
        # self.q4z = awg_channel(self.awg_tek, 4)
        # self.q5z = awg_channel(self.hdawg, 4)
        # self.q6z = awg_channel(self.hdawg, 5)
        # self.q1z.set_offset(0)
        # self.q2z.set_offset(0)
        # self.q3z.set_offset(0)
        # self.q4z.set_offset(0)
    ...
    def set_pulsed_mode(self):
        ...
        self.hdawg.set_offset(offset=0, channel=2)
        self.hdawg.set_offset(offset=0, channel=3)
    ...
    def setup_iq_channel_connections(self, exdir_db):
        ...
        self.iq_devices['iq_ex1'].name = 'ex1'  # mixer for ext for qubit 1
        ...
        self.iq_devices['iq_ex1'].sa = self.sa   # spectral analyzer for iq mixer 1
