...
def error_detection(device, qubit_ids, gate, phis=None, *extra_sweep_args, additional_metadata={}):
    '''
    quantum circuit
    ...
    # штука для контрольного сиквенсера
    ...
    channel_amplitudes_ = channel_amplitudes.channel_amplitudes(device, **{gate.metadata['carrier_name']:
                                                                               complex(gate.metadata['amplitude'])})
    ...
    class ParameterSetter:
        def __init__(self):
            ...
            self.pre_pulse1 = excitation_pulse.get_excitation_pulse(device=device, qubit_id=data_qubit_id,
                                                                    rotation_angle=np.pi / 2, sort='newest', gauss=True)

            self.pre_pulse2 = excitation_pulse.get_excitation_pulse(device=device, qubit_id=ancilla_qubit_id,
                                                                    rotation_angle=np.pi / 2, sort='newest', gauss=True)
        ...
        def return_hdawg_program(self, ex_seq):
            ...
            # делаем Rz(пи/2) и Rz(пи/2), Rz(3пи/2) импульс phase1_CZ - однокубитная фаза после CZ гейта
            phase1_CZ = 360 - 0.43374372 * 180
            for phase in [90, 180, 270, phase1_CZ]:
            ...
            phase2_CZ = 360 - 6.03341667 * 180
            for phase in [90, 180, 270, phase2_CZ]:
                if (ex_seq.params['sequencer_id'] == self.ancilla_qubit_sequence.params['sequencer_id']) and (
                        ex_seq.awg.device_id == self.ancilla_qubit_awg.device_id):
            ...
            play_part = textwrap.dedent('''
                                    // Table entry play part
                                        ...
                                        // do error
                                        executeTableEntry(1);
                                        executeTableEntry(14+variable_register2);

                                        // Two Hadamard gates on both qubits
                                        executeTableEntry(1);
                                        executeTableEntry(6);
                                        executeTableEntry(1);
                                        ...
                                        resetOscPhase();
                                    ''')
            # это делаем Z ошибку и получаем прямую, тк сильные
            # play_part = textwrap.dedent('''
            #                         // Table entry play part
            #                             executeTableEntry(0);
            #
    ...
    measurement_type = 'error_detection_rsf'
