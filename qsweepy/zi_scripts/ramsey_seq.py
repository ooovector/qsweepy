...
class RAMSEYSequence:
    def __init__(self, sequencer_id, awg, tail_length, readout_delay=0, awg_amp=1, pre_pulses = [],
        ...
        self.params = dict(sequencer_id=sequencer_id, qubit_channel=0,
                           ...
                           length__reg=length__reg, resudual__reg=resudual__reg, nco_id=sequencer_id*4,
...
var switch_value = getUserReg(resudual__reg);
...
while (true) {{
    ...
    pause_length = getUserReg(length__reg);
    //switch_value = getUserReg(resudual__reg); 
    waitDIOTrigger();
    //setDIO(0);
    //resetOscPhase();
    ''')
