...
from ctypes import *
from _Spectrum_M3i2132.errors import errors as _spcm_errors
import _Spectrum_M3i2132.regs as _spcm_regs
...
class Spectrum_M3i2132(Instrument):
	...
	Usage:
	Initialize with
	<name> = instruments.create('name', 'Spectrum_M3i2132')
	...
	def __init__(self, name):
		...
		# add parameters
		self.add_parameter('software_averages', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('software_nums_multi', flags=Instrument.FLAG_GETSET, type=int)
		...
		self.add_parameter('input_path_ch0', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('input_path_ch1', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('clock', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('reference_clock', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('nop', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('nums', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('loops', flags=Instrument.FLAG_GETSET, type=int)
		self.add_parameter('trigger_ext0_level0', flags=Instrument.FLAG_SET, type=int, minval=-5000, maxval=5000, unit='mV')
		self.add_parameter('trigger_ext0_level1', flags=Instrument.FLAG_SET, type=int, minval=-5000, maxval=5000, unit='mV')
...
########################################
	...
	def init_channel0_single_mode(self, memsize=2048, posttrigger=1024, amp=500, offs=0):
		...
		self._set_param(_spcm_regs.SPC_TRIG_EXT0_PULSEWIDTH, 0);
		...
		self._set_param(_spcm_regs.SPC_MEMSIZE, memsize)
		self._set_param(_spcm_regs.SPC_POSTTRIGGER, posttrigger)
		self._set_param(_spcm_regs.SPC_PATH0, 0) # zofo
		self._set_param(_spcm_regs.SPC_AMP0, amp)
		self._set_param(_spcm_regs.SPC_OFFS0, offs)
	...
	def set_ext_clock(self, rate):
		...
		self._set_param(_spcm_regs.SPC_REFERENCECLOCK, int(rate))
	...
	def init_channel0_multiple_recording(self, nums = 1024, segsize=128, posttrigger=1024, amp=500, offs=0):
		...
		self._set_param(_spcm_regs.SPC_CARDMODE, _spcm_regs.SPC_REC_FIFO_MULTI)
		#self._spcm_win32.SetParam32(self._spcm_win32.handel, _spcm_regs.SPC_LOOPS, 1)
		self._set_param(_spcm_regs.SPC_SEGMENTSIZE, segsize)
		self._set_param(_spcm_regs.SPC_MEMSIZE, memsize)
		self._set_param(_spcm_regs.SPC_POSTTRIGGER, posttrigger)
		self._set_param(_spcm_regs.SPC_AMP0, amp)
		self._set_param(_spcm_regs.SPC_OFFS0, offs)
	...
	def init_channel01_multiple_recording(self, nums=1024, segsize=128, posttrigger=64, amp0=500, offs0=0, amp1=500, offs1=1):
		'''
		Initiates the card in:
			Standard Multiple Recording mode
	...
	def do_set_software_nums_multi(self, software_nums_multi):
		self.software_nums_multi = software_nums_multi
		
	def do_get_software_nums_multi(self):
		return self.software_nums_multi
		
	def do_set_software_averages(self, software_averages):
		self.software_averages = software_averages
		
	def do_get_software_averages(self):
		return self.software_averages
		
	def do_set_nums(self, nums):
		'''
		Initiates the card in:
			Standard Multiple Recording mode
			Using Channel 0 and 1
		...
		self.nums = nums
		memsize = nums*self.get_nop() # Note: memsize is defined per channel
		self.set_memsize(memsize)
	
	def do_get_nums(self):
		self.nums = self.get_memsize()/self.get_nop()
...
################################
	...
	def start_with_trigger_and_waitready(self):
		...
		self._buffer_setup()
		self._set_param(_spcm_regs.SPC_M2CMD,
			_spcm_regs.M2CMD_CARD_START | _spcm_regs.M2CMD_CARD_ENABLETRIGGER | _spcm_regs.M2CMD_CARD_WAITREADY | _spcm_regs.M2CMD_DATA_WAITDMA)
	...
	def waitready(self):
		...
		err = self._set_param(_spcm_regs.SPC_M2CMD, _spcm_regs.M2CMD_CARD_WAITREADY |  _spcm_regs.M2CMD_DATA_WAITDMA)
		return err
...
### and trigger with ext. sources
	def set_trigger_ext0_AND_ext1(self):
		self._set_param(_spcm_regs.SPCM_X0_MODE, _spcm_regs.SPCM_XMODE_TRIGIN)
		self._set_param(_spcm_regs.SPC_TRIG_EXT1_MODE, _spcm_regs.SPC_TM_HIGH)
		self._set_param(_spcm_regs.SPC_TRIG_ANDMASK, _spcm_regs.SPC_TMASK_EXT0 | _spcm_regs.SPC_TMASK_EXT1)
		self._set_param(_spcm_regs.SPC_TRIG_ORMASK, _spcm_regs.SPC_TMASK_EXT0 | _spcm_regs.SPC_TMASK_EXT1)

	def set_trigger_ext0(self):
		self._set_param(_spcm_regs.SPC_TRIG_ANDMASK, _spcm_regs.SPC_TMASK_EXT0)
		self._set_param(_spcm_regs.SPC_TRIG_ORMASK, _spcm_regs.SPC_TMASK_EXT0)

	def set_trigger_ext1(self):
		logging.debug(__name__ + ' :Set trigger mode pos ext1')
		self._set_param(_spcm_regs.SPCM_X0_MODE, _spcm_regs.SPCM_XMODE_TRIGIN)
		self._set_param(_spcm_regs.SPC_TRIG_EXT1_MODE, _spcm_regs.SPC_TM_HIGH)
		self._set_param(_spcm_regs.SPC_TRIG_ANDMASK, _spcm_regs.SPC_TMASK_EXT1)
		self._set_param(_spcm_regs.SPC_TRIG_ORMASK, _spcm_regs.SPC_TMASK_EXT1)
		
### trigger levels
	def do_set_trigger_ext0_level0(self, value):
		self._set_param(_spcm_regs.SPC_TRIG_EXT0_LEVEL0, value)

	def do_set_trigger_ext0_level1(self, value):
		self._set_param(_spcm_regs.SPC_TRIG_EXT0_LEVEL1, value)
...
#######################
	...
	def _buffer_setup(self):
		...
		numchannels = self._get_param(_spcm_regs.SPC_CHCOUNT)
	...
	def readout_raw_buffer(self, nr_of_channels=1):
		...
		err = self._spcm_win32.SetParam32(self._spcm_win32.handel, _spcm_regs.SPC_M2CMD,
			_spcm_regs.M2CMD_DATA_WAITDMA)# | _spcm_regs.M2CMD_CARD_WAITREADY)
		if (err!=0):
	...
	def readout_singlechannel_multimode_bin(self):
		...
		if data == 'timeout':
			return data
		data = numpy.frombuffer(data, numpy.int8)
	...
	def measure(self):
		...
		for i in range(self.software_averages):
			for j in range(self.software_nums_multi):
				...
				data[:,j*lnumber_of_segments:(j+1)*lnumber_of_segments,:] = \
						self.readout_doublechannel_multimode_bin()/float(self.software_averages)
				self.stop()
...
### test run
	...
	def test(self, memsize=2048, posttrigger=1024, amp=500):
		...
		'''
		self.init_channel0_single_mode(memsize=memsize, posttrigger=posttrigger, amp=amp, offs=0)
		print( 'starting card and waiting for trigger')
		self.start_with_trigger_and_waitready()
		print( "received trigger")
		self.data = self.readout_singlechannel_singlemode_float()
		return self.data
...
### timing
	...
	def do_set_nop(self, lSegsize):
		...
		self._set_param(_spcm_regs.SPC_MEMSIZE, lSegsize*self.nums)
	...
	def do_set_loops(self, lLoops):
		'''
		Sets the number of times the ring buffer is filled.
		#Values other than 1 are effectively ignored.
		...
		'''
		#lLoops = 1
		logging.debug(__name__ + ' : Set loop count to %s' % lLoops)
		self._set_param(_spcm_regs.SPC_LOOPS, lLoops)

	def do_get_loops(self):
		'''
		Get number of times application ring buffer is filled.
...
### channel parameters
	...
	def do_get_input_path_ch0(self):
		...
		Output:
			path (int): selected path
		'''
		self._get_param(_spcm_regs.SPC_PATH0)
	...
	def do_get_input_path_ch1(self):
		...
		Output:
			path (int): selected path
		'''
		self._get_param(_spcm_regs.SPC_PATH1)
...
### clock
	...
	def do_set_clock(self, rate):
		'''
		defines the sampling rate in Hz for internal
		sample rate generation

		Input:
			rate (int) : sample rate in Hz, minimum is 10MSPS for M3i cards
