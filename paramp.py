...
class paramp:
	...
	# maximize SNR of S21 at given frequency, excitation power and bandwidth
		...
		params = ((self.pump_src.set_frequency, self.pump_src.get_frequency()),
				  ...
				  (self.bias_src.set_current if not self.voltage_setter else self.voltage_setter, self.bias_src.get_current() if not self.voltage_setter else self.voltage_getter()))
	...
	def load_calibration(self, path, name):
		...
		self.calib_bias = calib_file['Bias'][2]
		self.calib_pumpfreq = calib_file['Pump frequency'][2]
		self.calib_pumppower = calib_file['Pump power'][2]
		self.calib_targetfreq = calib_file['Pump power'][1][0]
